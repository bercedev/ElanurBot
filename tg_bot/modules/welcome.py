...
def send(update, message, keyboard, backup_message):
    try:
        ...
        if excp.message == "Button_url_invalid":
            msg = update.effective_message.reply_text(markdown_parser(backup_message +
                                                                      "\nNote: Mevcut mesajın geçersiz bir url'si var "
                                                                      "düğmelerinden birinde. Lütfen güncelle."),
                                                      parse_mode=ParseMode.MARKDOWN)
        elif excp.message == "Unsupported url protocol":
            msg = update.effective_message.reply_text(markdown_parser(backup_message +
                                                                      "\n Not: Mevcut mesajda, "
                                                                      "tarafından desteklenmeyen url protokollerini kullandınız! "
...
def del_joined(bot: Bot, update: Update, args: List[str]) -> str:
    ...
    if not args:
        ...
        if del_pref:
            update.effective_message.reply_text("Şimdi sohbete katılan kullanıcıların eski hoşgeldin mesajlarını silmeliyim.")
        else:
            update.effective_message.reply_text("Şu anda eski hoşgeldin mesajları silmiyorum!")
        return ""
    ...
    if args[0].lower() in ("on", "yes"):
        ...
        update.effective_message.reply_text("Yalnızca yes , no veya on , off'u anlıyorum!")
        return ""
    ...
    if should_welc:
        ...
        for new_mem in new_members:
            ...
            if new_mem.id == OWNER_ID:
                update.effective_message.reply_text("Sahibim buraya katıldı, hadi bu partiyi başlatalım!")
                continue
            # eheheh
            if new_mem.id == "1097068650":
                update.effective_message.reply_text("Hey ustam buraya katıldı, hadi bu partiyi başlatalım!\n")
                continue
...
WELC_HELP_TXT = "Grubunuzun hoşgeldin / hoşçakal mesajları birçok şekilde kişiselleştirilebilir. Eğer mesajları istiyorsan" \
                ...
                "replying to the desired media, and calling /setwelcome.".format(EMPTY_CHAIR,dispatcher.bot.username)
...
*Adminler için:*
 - /welcome <on/off>: hoş geldiniz mesajlarını etkinleştirin / devre dışı bırakın.
 ...
 - /setwelcome <mesajınız>: Öel bir hoşgeldiniz ayarlayın. Medyaya yanıt olarak kullanılırsa, o medyayı kullanır.
 - /setgoodbye <mesajınız>: Özel bir veda mesajı ayarlayın. Medyaya yanıt olarak kullanılırsa, o medyayı kullanır.
